define(function(require, exports, module) {
    ...
    dySystem.config(["w5cValidatorProvider", function (w5cValidatorProvider) {
        ...
        w5cValidatorProvider.setRules({
            ...
            password: {
                required: "请输入密码",
                minlength: "密码长度不符合6-15位限制",
                maxlength: "密码长度不符合6-15位限制",
                nospecialstr:"密码不能有特殊字符"
            },
            cPassword: {
                required: "请再输入密码",
                repeat: "两次输入的密码不一致",
    ...
    dySystem.controller('MobileMain', function($scope, $http, ngDialog, $timeout,$injector,$cookieStore) {
        //$.cookie('rebackUrl',"");
        $cookieStore.put("rebackUrl","");
        //弹窗提示到pc端操作
    ...
    //新的控制器开始
    //注册或登录
    dySystem.controller('regloginCtrl',['$scope','$http','$injector','$location','$cookieStore',function($scope,$http,$injector,$location,$cookieStore){
        ...
        vm.regloginForm = function () {
            ...
            var callback = function(_data){
                if(_data.code == "100"){
                    ...
                    $cookieStore.put('accounts',$scope.register.account);
                    if(_data.code == "1"){
   ...
   /**
    ...
    //注册(先不做推荐人)
    dySystem.controller("register2Ctrl",function($scope,$http,$injector,$cookieStore){
        //注册账号
        $scope.account = account;
        $scope.register2 = {};
        $scope.register2.account = account;
        var vm = $scope.vm = { htmlSource: "" };        
        vm.validateOptions = {  //每个表单的配置，如果不设置，默认和全局配置相同
            blurTrig: true,
            showError : false,
        ...
        $scope.getPhoneCode = function(){
            ...
            var callback = function(_data){
                if(_data.code == "200"){
                    $scope.countDown();
                    $scope.smsNotice = "已向手机<span>"+$scope.account+"</span>发送短信，请输入短信验证码完成注册。"
    ...
    //登录
    dySystem.controller('loginCtrl',['$scope','$http','$injector','$cookieStore',function($scope,$http,$injector,$cookieStore){
        //登录账号
        $scope.account = account;
        //登录方式，默认验证码登录
        ...
        $scope.getPhoneCode = function(){
            ...
            var callback = function(_data){
                if(_data.status == "200"){
                    $scope.countDown();
                    $scope.smsNotice = "已向手机<span>"+$scope.account+"</span>发送短信，请输入短信验证码完成登录。"
                    $scope.getCodeVal="发送完成";
                    $scope.showError();
                }else{
                    $scope.showError(_data.description);
    ...
    //登录2
    dySystem.controller('login2Ctrl',['$scope','$http','$injector','$cookieStore',function($scope,$http,$injector,$cookieStore){
        $scope.account = account;
        $scope.login = {"logintype":2};
    ...
    dySystem.controller('searchPwdphoneCtrl',['$scope','$http','$injector',function($scope,$http,$injector){
        ...
        $scope.getPhoneCode = function(){
            ...
            var callback = function(_data){
                if(_data.code == "200"){
                    $scope.countDown();
                    $scope.smsNotice = "已向手机<span>"+_data.data+"</span>发送短信，请输入短信验证码完成登录。"
                    $scope.getCodeVal="发送完成";
                    $scope.showError();
                }else{
                    $scope.showError(_data.description);
    ...
    dySystem.controller('searchPwdemailCtrl',['$scope','$http','$injector',function($scope,$http,$injector){
        ...
        $scope.getEmailCode = function(){
            ...
            var callback = function(_data){
                if(_data.code == "200"){
                    $scope.countDown();
                    $scope.smsNotice = "已向邮箱<span>"+_data.data.email+"</span>发送邮件，请输入邮件验证码完成登录。"
                    $scope.getCodeVal="发送完成";
                    $scope.showError();
                }else{
                    $scope.showError(_data.description);
